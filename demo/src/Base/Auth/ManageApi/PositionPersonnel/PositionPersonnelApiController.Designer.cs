...
namespace Agebull.Common.Organizations.WebApi.Entity
{
    partial class PositionPersonnelApiController
    {
        ...
        public ApiArrayResult<EasyComboValues> ComboData()
        {
            ...
            var combos = datas.Select(p => new EasyComboValues(p.Id, p.Appellation)).ToList();
        ...
        public void SetKeywordFilter(LambdaItem<PositionPersonnelData> filter)
        {
            ...
            {
                filter.AddAnd(p =>p.Appellation.Contains(keyWord) || 
        ...
        protected void DefaultReadFormData(PositionPersonnelData data, FormConvert convert)
        {
            ...
            data.OrganizePositionId = convert.ToLong("OrganizePositionId");
            //备注
