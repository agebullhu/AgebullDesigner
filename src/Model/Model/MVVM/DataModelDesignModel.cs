    ...
    public sealed class DataModelDesignModel : ModelBase
    {
        ...
        bool CheckType(Type source, Type target,bool signle)
        {
            if (target == typeof(ConfigBase))
                return true;
            if (source == typeof(SolutionConfig))
                return true;

            if (source == target || source.IsSubclassOf(target))
                return true;
            if (signle)
                return false;
            if (source == typeof(EntityConfig))
            {
                return target == typeof(FieldConfig);
            }
            ...
            if (source == typeof(EntityClassify))
            {
                return target == typeof(EntityConfig) ||
                     target == typeof(EntityClassify) ||
                     target == typeof(FieldConfig) ||
                     target == typeof(ProjectConfig);
            }
            if (source == typeof(ProjectConfig))
            {
                return target == typeof(EntityConfig) ||
                     target == typeof(EntityClassify) ||
                     target == typeof(FieldConfig) ||
                     target == typeof(ModelConfig) ||
                     target == typeof(CommandConfig) ||
        ...
        internal static void LoadUserScreen()
        {
            ...
            var path = Path.Combine(GlobalConfig.RootPath, "Config", "screen.json");
            if (!File.Exists(path))
                return;
