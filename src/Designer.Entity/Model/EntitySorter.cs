...
namespace Agebull.EntityModel.Config
{
    ...
    public class EntitySorter: ConfigModelBase
    {
        ...
        public void SortFieldByIndex(bool reset)
        {
            ...
            foreach (var field in columns.OrderBy(p => p.Index).ToArray())
            {
                if (reset)
            ...
            foreach (var field in dcs.OrderBy(p => p.Index).ToArray())
            {
                if (reset)
                    field.Option.Index = ++idx;
                Entity.Properties.Add(field);
    ...
    /// 实体排序器
    /// </summary>
    public class ModelSorter : ConfigModelBase
    {
        /// <summary>
        /// 表结构对象
        /// </summary>
        ...
        public void SortByGroup()
        {
            var pc = Model.Properties.FirstOrDefault(p => p.IsCaption);
            var pk = Model.PrimaryColumn;
            var columns = Model.Properties.Where(p => !p.IsPrimaryKey && !p.IsCaption).OrderBy(p => p.Index).ToArray();
            Model.Properties.Clear();
            var idx = Model.ColumnIndexStart;
            if (pk != null)
            {
                pk.Option.Index = ++idx;
        ...
        public void SortFieldByIndex(bool reset)
        {
            var columns = Model.Properties.Where(p => !p.IsDiscard && !p.IsPrimaryKey).OrderBy(p => p.Index).ToList();
            var dcs = Model.Properties.Where(p => p.IsDiscard).OrderBy(p => p.Index).ToList();
        ...
        public void SortField()
        {
            var pc = Model.Properties.FirstOrDefault(p => p.IsCaption);
            var pk = Model.PrimaryColumn;
            var columns = Model.Properties.Where(p => !p.IsDiscard && !p.IsPrimaryKey && !p.IsCaption).OrderBy(p => p.Index).ToList();
            var dcs = Model.Properties.Where(p => p.IsDiscard).OrderBy(p => p.Index).ToList();
            var idx = Model.ColumnIndexStart;
            foreach (var field in columns)
            {
        ...
        public void SortField(bool reInsert)
        {
            var columns = Model.Properties.ToArray();
            if (reInsert)
            {
                Model.Properties.Clear();
            }
