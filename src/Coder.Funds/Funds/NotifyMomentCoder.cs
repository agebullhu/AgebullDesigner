...
{{
	...
	server_message_send(cmd_arg);");
        ...
        private static void NotifyBusinessHCode(NotifyItem item, StringBuilder code)
        {
...
{{
	...
	{{
        ...
        private static void NotifyBusinessCppCode(NotifyItem item, StringBuilder code)
        {
            var entity = item.LocalEntity;
            code.AppendLine($@"/*------------------------------------------------------
